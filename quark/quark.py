...
def download_images_from_quark(folder_name: str, desc_save_dir: str, logger):
    with QuarkClient() as client:
        # 搜索文件
        results = client.search_files(folder_name, size=1)
        logger.info('download_images', f"找到 {len(results['data']['list'])} 个文件")
        ...
        logger.info('download_images', file_ids)
...
if __name__ == "__main__":
    ...
    download_images_from_quark(folder_name='0104相机', desc_save_dir='input_images', logger=logger)
